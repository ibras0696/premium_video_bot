...
class CrudeUser:
    ...
    async def add_user(self,
        ...
        :param referral_id: Реферальный айди пригласившего в боте
        :return: True при существовании пользователя и False при добавлении
        """
        async with self.session() as conn:
            try:
                # Исправление реферального айди в случае объекта str
                if isinstance(referral_id, str):
                    referral_id = int(referral_id)
                # Исправление Телеграм айди в случае объекта str
                ...
                # Добавление по реферальной ссылке
                if isinstance(referral_id, int):
                    user = User(telegram_id=telegram_id, user_name=user_name, referral_id=referral_id)
                else:
    ...
    # Функция для получения всех пользователей по реферальному айди
    async def get_users_by_referral(self, referral_id: int) -> list[User] | bool:
        """
        Получить всех пользователей с заданным referral_id
        :param referral_id: int - реферальный айди
        :return: список пользователей | False при отсутствии
        """
        async with self.session() as conn:
            try:
                result = await conn.execute(select(User).where(User.referral_id == referral_id))
                ...
                raise SQLAlchemyError(f'Ошибка при получении пользователей по referral_id={referral_id}: {ex}')
            except Exception as ex:
    ...
    # Функция для подсчета общего количества пользователей
    async def count_users_by_referral(self, referral_id: int) -> int:
        """
        Функция для подсчета общего количества пользователей
        :param referral_id: реферальный айди пригласившего
        :return: Количество пользователей по реф айди
        """
        async with self.session() as conn:
            try:
                result = await conn.execute(select(func.count()).where(User.referral_id == referral_id))
                return result.scalar_one()
            except Exception as ex:
                raise Exception(f'Ошибка при подсчете пользователей с referral_id={referral_id}: {ex}')
...
class CrudeSubscriptions:
    ...
    async def add_subscription(self,
        ...
        async with self.session() as conn:
            try:
                # Выполняем запрос к таблице Subscriptions — выбираем подписку с данным telegram_id и тарифом plan.
    ...
    async def extend_subscription(self, telegram_id: int, plan: str, days: int) -> bool:
        """
        Функция для увелечения подписки
        :param telegram_id:
        :param plan: Тариф
    ...
    async def reduce_subscription(self, telegram_id: int, plan: str, days: int) -> bool:
        """
    ...
    async def get_all_user_subscriptions(self, telegram_id: int) -> list[Subscriptions] | bool:
        """
        Функция для получения данных о подписках определенного пользователя
        :param telegram_id: Телеграм айди
        :return: Список объектов Subscriptions определенного человека если ничего нет то False
        """
        async with self.session() as conn:
            try:
...
class CrudePayments:
    ...
    основные параметры таблицы
    id, telegram_id, registered_at, plan, day_count, pay_sum, day, month, year
    """
    def __init__(self):
        self.session: async_sessionmaker = async_session
